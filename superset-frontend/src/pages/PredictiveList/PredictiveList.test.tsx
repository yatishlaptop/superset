...
const datasetsInfoEndpoint = 'glob:*/api/v1/predictive/_info*';
const datasetsOwnersEndpoint = 'glob:*/api/v1/predictive/related/owners*';
const datasetsSchemaEndpoint = 'glob:*/api/v1/predictive/distinct/schema*';
const datasetsDuplicateEndpoint = 'glob:*/api/v1/predictive/duplicate*';
const databaseEndpoint = 'glob:*/api/v1/predictive/related/database*';
const datasetsEndpoint = 'glob:*/api/v1/predictive/?*';
...
describe('DatasetList', () => {
  ...
  it('renders', () => {
    expect(wrapper.find(PredictiveList)).toBeTruthy();
  });
  ...
  it('fetches info', () => {
    const callsI = fetchMock.calls(/predictive\/_info/);
    expect(callsI).toHaveLength(1);
  });

  it('fetches data', () => {
    const callsD = fetchMock.calls(/predictive\/\?q/);
    expect(callsD).toHaveLength(1);
    expect(callsD[0][0]).toMatchInlineSnapshot(
      `"http://localhost/api/v1/predictive/?q=(order_column:changed_on_delta_humanized,order_direction:desc,page:0,page_size:25)"`,
    );
  });

  it('does not fetch owner filter values on mount', async () => {
    await waitForComponentToPaint(wrapper);
    expect(fetchMock.calls(/predictive\/related\/owners/)).toHaveLength(0);
  });

  it('does not fetch schema filter values on mount', async () => {
    await waitForComponentToPaint(wrapper);
    expect(fetchMock.calls(/predictive\/distinct\/schema/)).toHaveLength(0);
  });
  ...
  it('calls the duplicate endpoint', async () => {
    ...
    expect(fetchMock.calls(/predictive\/duplicate/)).toHaveLength(1);
  });
...
describe('RTL', () => {
  async function renderAndWait() {
    const mounted = act(async () => {
      ...
      render(
        <QueryParamProvider>
          <PredictiveList {...mockedProps} user={mockUser} />
        </QueryParamProvider>,
